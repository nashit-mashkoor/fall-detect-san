...
# Loading onnx files
model = onnx.load('/content/fall-detect.onnx')
# Using tensorrt backend engine for onnx model
...
def get_all_features(ip_set, lstm_set):
    ...
    for i, ips in enumerate(ip_set):
        ...
        if lstm_set[i][0]:
          h_n = lstm_set[i][0][0].numpy().astype('float32')
          h_c = lstm_set[i][0][1].numpy().astype('float32')
          onnx_output = engine.run((xdata.numpy(), h_n, h_c))
        else:
          h_n = np.random.randn(2, 1, 32).astype('float32')
          h_c = np.random.randn(2, 1, 32).astype('float32')
          onnx_output = engine.run((xdata.numpy(),h_n, h_c))
        
       
        
        outputs = torch.tensor(onnx_output[2])
        lstm_set[i][0] = (torch.tensor(onnx_output[0]), torch.tensor(onnx_output[1]))
        
        print('TensorRT optimsed engine prediction time: {}'.format(time.time() - engine_time))
        # print(outputs)
        # print(type(outputs))
        # print(outputs.shape)
        # print(lstm_set[i][0])
