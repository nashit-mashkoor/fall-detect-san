...
def extract_keypoints_parallel(queue, args, self_counter, other_counter, consecutive_frames, event):
    ...
    while not event.is_set():
        ...
        print("Time to read frame: " + str(time.time() - start))
        if img is None:
        ...
        if args.coco_points:
            ...
            for box in bbox_list:
                ...
                cv2.imwrite("boxes/" + str(i) + ".jpg", img)
...
def show_tracked_img(img_dict, ip_set, num_matched, output_video, args):
    ...
    if output_video is None:
        if args.save_output:
            ...
            output_video = cv2.VideoWriter(filename='results/tracked'+vidname[-1][:-3]+'avi', fourcc=cv2.VideoWriter_fourcc(*'MP42'),
                                           fps=args.fps, frameSize=img.shape[:2][::-1])
            logging.debug(
                f'Saving the output video at result with {args.fps} frames per seconds')
        else:
            output_video = None
...
def alg2_sequential(queues, argss, consecutive_frames, event, model):
    result = []
    ...
    # print("Inference Start time: " + str(start_time))
        ...
        if not any(q.empty() for q in queues):
            ...
            if cv2.waitKey(1) == 27 or any(cv2.getWindowProperty(window_name, cv2.WND_PROP_VISIBLE) < 1 for window_name in window_names):
                if not event.is_set():
                    event.set()
            # if skip_counter % 2 == 0:
            # print("Len of dict_frames: " + str(len(dict_frames)))
            # print("dict_frames: " + str(dict_frames))
            ...
            if argss[0].num_cams == 1:
                ...
                # print("Valid Index")
                # print(valid1_idxs)
                # print(valid1_idxs, prediction)
                if prediction != 16:
                    dict_frames[0]["tagged_df"]["text"] += f" Pred: {activity_dict[prediction+5]}"
                    # print("Prediction on frame: " + str(activity_dict[prediction+5]))
                    # print("Pred time on frame: " + str(time.time() - start))
                    # print("------------------------")
                    result.append(activity_dict[prediction+5])
    ...
    if 'FALL' in values:
        fall_count = counts[np.where(values=='FALL')]
        # print("fall here")
        if fall_count > 4:
            # print("fall count > 4")
            consec_frames = True
            index = result.index('FALL')
            ...
            if consec_frames:
                # print("fall yes")
                prediction = 'FALL'
            elif not consec_frames and 'FALL Warning' in values:
                warning_count = counts[np.where(values == 'FALL Warning')]
                # print(warning_count)
                if warning_count > 9:
                    consec_frames = True
                    index = result.index('FALL Warning')
                    for i in range(index, index + 10):
                        if result[i] != 'FALL Warning':
                            consec_frames = False
                            break
                    if consec_frames:
                        prediction = 'FALL Warning'
            else:
                prediction = values[np.argmax(counts)]
        elif 'FALL Warning' in values:
            warning_count = counts[np.where(values == 'FALL Warning')]
            # print(warning_count)
            if warning_count > 9:
                consec_frames = True
            ...
            prediction = values[np.argmax(counts)]
    if 'FALL' not in values and 'FALL Warning' in values:
        warning_count = counts[np.where(values == 'FALL Warning')]
        print(warning_count)
        if warning_count > 9:
            consec_frames = True
            ...
            prediction = values[np.argmax(counts)]
    if 'FALL' not in values and 'FALL Warning' not in values:
        prediction = values[np.argmax(counts)]
    print("Pred: " + str(prediction))
    print("Total inference time: " + str(time.time()-start_time))
    # del model
    return
...
def get_all_features(ip_set, lstm_set, model):
    ...
    for i, ips in enumerate(ip_set):
        ...
        if i == 0:
            ...
            # print(outputs.data)
            # print(np.argsort(np.array(outputs.data)))
            # if outval == 0 or outval == 5:
            # prediction = torch.max(outputs.data, 1)[1][0].item()
            # elif :
            # print(prediction)
    ...
    # print("Final Result: " + str(predictions[0]))
